...
df_filtered['base_method'] = df_filtered['method'].apply(lambda x: 'IGES' if 'IGES' in x else 'MD_scikit')
...
fig.suptitle('IGES vs MD_scikit Performance', fontsize=14, y=0.98)

# Minimalistic color scheme
iges_color = '#2E86AB'  # Blue
...
for idx, dataset in enumerate(dataset_order):
    ...
    for mtry in mtry_labels:
        iges_row = dataset_data[(dataset_data['base_method'] == 'IGES') & 
                               (dataset_data['mtry_setting'] == mtry)]
        md_row = dataset_data[(dataset_data['base_method'] == 'MD_scikit') & 
                             (dataset_data['mtry_setting'] == mtry)]
        
        iges_mcc = iges_row['test_mcc_median'].iloc[0] if not iges_row.empty else 0
        md_mcc = md_row['test_mcc_median'].iloc[0] if not md_row.empty else 0
        
        iges_data.append(iges_mcc)
        md_data.append(md_mcc)
    
    # Create clean bars without patterns (MD_scikit on left, IGES on right)
    bars1 = ax.bar(x_positions - width/2, md_data, width,
                   label='MD_scikit', color=md_color, alpha=0.8,
                   edgecolor='white', linewidth=1)
    
    bars2 = ax.bar(x_positions + width/2, iges_data, width, 
                   label='IGES', color=iges_color, alpha=0.8, 
                   edgecolor='white', linewidth=1)
...
legend_elements = [
    plt.Rectangle((0,0),1,1, facecolor=md_color, alpha=0.8, label='MD_scikit'),
    plt.Rectangle((0,0),1,1, facecolor=iges_color, alpha=0.8, label='IGES')
]
...
plt.savefig('results/rf_comparison_plot.pdf', bbox_inches='tight')

# Show the plot
plt.show()

print("Visualization saved as 'results/rf_comparison_plot.png' and 'results/rf_comparison_plot.pdf'")
    ...
    for method in ['IGES', 'MD_scikit']:
        ...
        for mtry in ['1', '√d', 'd']:
            ...
            if not mtry_data.empty:
                ...
                print(f"  mtry={mtry}: MCC={mcc:.3f}, Depth={depth:.1f}, Leaves={leaves:.1f}, Time={time:.3f}s") 