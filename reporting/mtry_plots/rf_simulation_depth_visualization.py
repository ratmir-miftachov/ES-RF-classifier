...
# Read the data
df = pd.read_csv('results/rf_simulation_d100.csv')
...
fig.suptitle('Tree Depth: ES Random Forest (IGES) vs Deep Random Forest (d=100)', fontsize=14, y=0.98)
...
# Create depth plots for all 8 datasets
for dataset_idx, dataset in enumerate(dataset_order):
    ...
    for mtry in mtry_labels:
        ...
        es_score = es_row['depth_median'].iloc[0] if not es_row.empty else 0
        deep_score = deep_row['depth_median'].iloc[0] if not deep_row.empty else 0
    ...
    # Set fixed y-axis limits for depth
    ax.set_ylim(0, 25)
...
# Save the plot
plt.savefig('results/rf_d100_depth_comparison_plot.png', dpi=300, bbox_inches='tight')

# Show the plot
plt.show()

print("Depth visualization saved as 'results/rf_d100_depth_comparison_plot.png'")

# Print summary statistics focused on depth
print("\n" + "="*60)
print("DEPTH ANALYSIS SUMMARY (d=100)")
print("="*60)
    ...
    for method in ['ES random forest (IGES)', 'deep random forest']:
        ...
        for mtry in ['1', '√d', 'd']:
            ...
            if not mtry_data.empty:
                ...
                print(f"  mtry={mtry}: Depth={depth:.1f}, Leaves={leaves:.1f}, MCC={mcc:.3f}") 