...
# Read the data
df = pd.read_csv('results/rf_empirical_study.csv')
...
for dataset_idx, dataset in enumerate(dataset_order):
    ...
    for mtry in mtry_labels:
        ...
        es_score = es_row['mcc_test (mean)'].iloc[0] if not es_row.empty else 0
        deep_score = deep_row['mcc_test (mean)'].iloc[0] if not deep_row.empty else 0
...
# Save the plot
plt.savefig('results/rf_empirical_comparison_plot.png', dpi=300, bbox_inches='tight')

# Show the plot
plt.show()

print("Visualization saved as 'results/rf_empirical_comparison_plot.png'")
    ...
    for method in ['ES random tree', 'deep random tree']:
        ...
        for mtry in ['1', '√d', 'd']:
            ...
            if not mtry_data.empty:
                mcc = mtry_data['mcc_test (mean)'].iloc[0]
                acc = mtry_data['test_acc (mean)'].iloc[0]
                depth = mtry_data['mean_depth (mean)'].iloc[0]
                leaves = mtry_data['mean_n_leaves (mean)'].iloc[0]
